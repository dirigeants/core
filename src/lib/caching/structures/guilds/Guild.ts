...
export class Guild extends Structure {
   ...
   */
	public async modify(
		{ icon, splash, banner, ...options }: GuildModifyOptions,
   ...
   */
	...
	public async prune(options: GuildPruneOptions, requestOptions: RequestOptions = {}): Promise<number | null> {
		...
		if (options.dry) {
			const result = (await this.client.api.get(Routes.guildPrune(this.id), {
				...requestOptions,
				query: [...query]
			})) as { pruned: number };
		...
		const result = (await this.client.api.post(Routes.guildPrune(this.id), {
			...requestOptions,
			query: [...query]
